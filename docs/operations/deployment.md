...
2. Configurar `ZER_RPC_URL` para o `zerod` próprio, acessível por IP privado, loopback ou nome de serviço local. Testar assinatura de transação sem broadcast e confirmar que o nó aceita a chamada durante a sincronização. A aplicação rejeita RPC público como signer.
3. Para o quarto provedor DOGE, configurar `DOGE_BLOCKBOOK_API_KEY` no worker e validar `https://dogebook.nownodes.io/api/v2/address/{address}?details=txs` a partir da VM com um endereço público de teste. A chave vai no header `api-key`. Conferir também DGB Insight/Blockbook e comparar altura de bloco antes de concluir que a detecção está saudável.
