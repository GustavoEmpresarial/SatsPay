...
const pagePath = path.join(root, 'src/pages/DepositPage.tsx');

describe('Deposit — structure', () => {
  it('page module file exists', () => {
    expect(existsSync(pagePath)).toBe(true);
  });

  it('is wired in App routes', () => {
    expect(app).toContain("path=\"/deposit\"");
    expect(app).toContain("DepositPage");
  });
  ...
  it('picker lists active coins and shows paused networks as disabled rows', () => {
    ...
    expect(src).toContain('depositWithdrawPausedCoinList');
