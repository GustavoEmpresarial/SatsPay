//! Non-admin bearer gets 403 on admin routes.
...
async fn admin_route_rejects_plain_user(pool: PgPool) {
    let state = common::test_state(pool);
    let email = format!("user-{}@bitcosats.test", Uuid::new_v4());
    ...
    // Any admin-gated GET — pending withdrawals list.
    let response = api_http::app_without_metrics(state)
        .oneshot(
            axum::http::Request::builder()
                .uri("/v1/admin/pending-withdrawals")
    ...
    assert_eq!(err["error"], "admin access required");
}
