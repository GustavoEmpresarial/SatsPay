//! Authenticated GET /v1/wallet returns personal wallets after register.
...
async fn list_wallets_after_register(pool: PgPool) {
    let state = common::test_state(pool);
    let email = format!("wlt-{}@bitcosats.test", Uuid::new_v4());
    ...
    let list = wallets.as_array().expect("wallet array");
    assert!(!list.is_empty());
    assert!(list.iter().any(|w| w["coin"] == "BTC" && w["kind"] == "PERSONAL"));
}
