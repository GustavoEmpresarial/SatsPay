//! GET /v1/auth/security-logs with bearer.
...
async fn security_logs_returns_logs_array(pool: PgPool) {
    let state = common::test_state(pool);
    let email = format!("sec-{}@bitcosats.test", Uuid::new_v4());
    ...
    assert!(body["logs"].as_array().is_some());
}
