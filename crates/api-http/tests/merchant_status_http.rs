//! GET /v1/merchant/status — new users are auto-APPROVED (migration 0013).
...
async fn merchant_status_auto_approved(pool: PgPool) {
    let state = common::test_state(pool);
    let email = format!("merch-{}@bitcosats.test", Uuid::new_v4());
